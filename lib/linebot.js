    ...
    body.events.forEach(function(event) {
      ...
      process.nextTick(function() {
        if(event.type === 'postback'){
          // like bootbot package here i am merging postback type and data
          // by using this we can listen separately for each emit like postback:SUBSCRIBE
          that.emit(event.type+":"+event.postback.data, event)
        }else that.emit(event.type, event);
      });
    });
  }
